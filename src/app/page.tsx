...
import { downloadImage, downloadSimpleImage, importCsvData, exportMaterialList } from '../utils/imageDownloader';
...
// 导入 Google Analytics 追踪函数
import { 
  trackImageUpload, 
  trackPixelation, 
  trackDownload, 
  trackExportMaterialList,
  trackColorSystemChange 
} from '../lib/analytics';
...
export default function Home() {
  ...
  const [granularity, setGranularity] = useState<number>(52);
  const [granularityInput, setGranularityInput] = useState<string>("52");
  ...
  const processFile = (file: File) => {
    ...
    if (fileExtension === 'csv') {
      ...
      // 处理图片文件 - 先显示裁切界面
      trackImageUpload(file.size); // 追踪图片上传
  ...
  const handleConfirmParameters = () => {
    ...
    if (granularityChanged || similarityChanged) {
      // 追踪像素化生成
      if (gridDimensions) {
        trackPixelation(`${gridDimensions.N}x${gridDimensions.M}`);
      }
  ...
  // --- Download function (ensure filename includes palette) ---
    ...
    // --- Simple download function (grid only with watermark) ---
    const handleSimpleDownloadRequest = () => {
        trackDownload('simple'); // 追踪简洁版下载
        downloadSimpleImage({
          mappedPixelData,
          gridDimensions,
          colorCounts,
  ...
  return (
    ...
    {/* 紫色渐变背景 */}
    <div className="min-h-screen p-4 sm:p-6 flex flex-col items-center bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 font-[family-name:var(--font-geist-sans)] overflow-x-hidden">
      {/* 标题区域 */}
        ...
        <div className="py-8">
          ...
          <div className="flex justify-center mb-4">
            <div className="relative">
              ...
              {/* 图标容器 */}
              <div className="relative bg-white bg-opacity-90 backdrop-blur-sm rounded-2xl p-4 shadow-2xl">
                {/* 拼豆网格图标 - 3x3网格 */}
                <svg width="64" height="64" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
                  {/* 绘制9个圆形拼豆 */}
                  <circle cx="16" cy="16" r="8" fill="#FF6B9D" />
                  <circle cx="32" cy="16" r="8" fill="#C084FC" />
                  <circle cx="48" cy="16" r="8" fill="#60A5FA" />
                  <circle cx="16" cy="32" r="8" fill="#A78BFA" />
                  <circle cx="32" cy="32" r="8" fill="#F472B6" />
                  <circle cx="48" cy="32" r="8" fill="#FB923C" />
                  <circle cx="16" cy="48" r="8" fill="#34D399" />
                  <circle cx="32" cy="48" r="8" fill="#FBBF24" />
                  <circle cx="48" cy="48" r="8" fill="#F87171" />
                  {/* 添加中心小孔效果 */}
          ...
          <p className="text-base sm:text-lg text-white text-opacity-90 drop-shadow-md">
            轻松将图片转换为拼豆图案，一键生成物料清单
          </p>
        </div>
      </header>
      ...
      <main ref={mainRef} className="w-full md:max-w-4xl flex flex-col items-center space-y-5 sm:space-y-6 relative overflow-hidden">
        ...
        <div
          ...
          className={`bg-white rounded-xl p-6 sm:p-8 text-center ${isMounted ? 'cursor-pointer hover:shadow-2xl' : 'cursor-wait'} transition-all duration-300 w-full md:max-w-md flex flex-col justify-center items-center shadow-lg`}
          style={{ minHeight: '130px' }}
        >
          {/* 图标 */}
          <svg xmlns="http://www.w3.org/2000/svg" className="h-10 w-10 sm:h-12 sm:w-12 text-purple-400 mb-2 sm:mb-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1}>
             <path strokeLinecap="round" strokeLinejoin="round" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
          </svg>
          {/* 文字 */}
          <p className="text-xs sm:text-sm text-gray-600">拖放图片到此处，或<span className="font-medium text-purple-600">点击选择文件</span></p>
          <p className="text-xs text-gray-400 mt-1">支持 JPG, PNG 图片格式，或 CSV 数据文件</p>
          <p className="text-xs text-purple-500 mt-2 font-medium">💡 建议：白色简洁背景，尽量使图像充满画布</p>
        </div>

        {/* 提示框 - 白色卡片 */}
        {!originalImageSrc && (
          <div className="w-full md:max-w-md bg-white p-4 rounded-xl shadow-lg">
            {/* 图标和文字 */}
            <p className="text-xs text-gray-700 flex items-start">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1.5 flex-shrink-0 text-purple-500 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              {/* 文字 */}
              <span className="text-gray-700">小贴士：使用像素图进行转换前，请确保图片的边缘吻合像素格子的边界线，这样可以获得更精确的切割效果和更好的成品。</span>
            </p>
          </div>
        )}
        ...
        {originalImageSrc && (
          <div className="w-full flex flex-col items-center space-y-5 sm:space-y-6">
            ...
            {!isManualColoringMode && (
              /* 白色卡片控制面板 */
              <div className="w-full md:max-w-2xl grid grid-cols-1 sm:grid-cols-2 gap-4 bg-white p-4 sm:p-5 rounded-xl shadow-lg">
                {/* 横轴切割数量 */}
                <div className="flex-1">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2 mb-2">
                      ...
                      <button
                        onClick={() => {
                          setGranularityInput("104");
                          setGranularity(104);
                        }}
                        className={`flex-1 px-2 py-1 text-xs rounded border transition-colors ${
                          granularityInput === "104"
                            ? 'bg-purple-500 text-white border-purple-500'
                            : 'bg-white text-gray-700 border-gray-300 hover:border-purple-400'
                        }`}
                      >
                    ...
                    <input
                      ...
                      value={granularityInput}
                      onChange={handleGranularityInputChange}
                      className="w-full p-1.5 border border-gray-300 rounded-md text-sm focus:ring-purple-500 focus:border-purple-500 h-9 shadow-sm bg-white text-gray-900 placeholder-gray-400"
                      min="10"
                      max="300"
                    />
                  </div>
                ...
                <div className="flex-1">
                    {/* 标签 */}
                    <label htmlFor="similarityThresholdInput" className="block text-xs sm:text-sm font-medium text-gray-700 mb-1.5 sm:mb-2">
                        颜色合并阈值 (0-100):
                    </label>
                    <div className="space-y-2">
                      {/* 滑动条 */}
                ...
                <div className="sm:col-span-2 grid grid-cols-2 gap-2">
                  <button
                    ...
                    className="h-9 bg-purple-500 hover:bg-purple-600 text-white text-sm rounded-md transition-colors duration-200 shadow-sm"
                  >
                    应用
                  </button>
                  <button
                    ...
                    className="inline-flex items-center justify-center h-9 text-sm rounded-md border border-purple-200 bg-purple-50 text-purple-700 hover:bg-purple-100 transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    一键去背景
                  </button>
                </div>
                ...
                <div className="sm:col-span-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <select
                      ...
                      className="w-full p-1.5 border border-gray-300 rounded-md text-sm focus:ring-purple-500 focus:border-purple-500 h-9 shadow-sm bg-white text-gray-900"
                    >
                      <option value={PixelationMode.Dominant} className="bg-white text-gray-900">卡通 (主色)</option>
                      <option value={PixelationMode.Average} className="bg-white text-gray-900">真实 (平均)</option>
                    </select>
                  </div>
                </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {colorSystemOptions.map(option => (
                      <button
                        key={option.key}
                        onClick={() => {
                          trackColorSystemChange(option.key); // 追踪色号系统切换
            ...
            <div className="w-full md:max-w-2xl">
              ...
              {isManualColoringMode && mappedPixelData && gridDimensions && (
                <div className="w-full mb-4 p-3 bg-white rounded-xl shadow-lg">
                  <div className="flex justify-center">
                    <div className="bg-purple-50 border border-purple-200 rounded-lg p-2 flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-3 text-xs text-gray-700 w-full sm:w-auto">
              ...
              <div className="bg-white p-4 rounded-xl shadow-lg">
                ...
                {gridDimensions && gridDimensions.N > 100 && (
                  <div className="mb-3 p-2 bg-purple-50 border border-purple-200 rounded-lg text-xs text-purple-700 text-center">
                 ...
                 {/* 内部容器背景 - 允许水平滚动以适应大画布 */}
                <div className="flex justify-center mb-3 sm:mb-4 bg-gray-50 p-2 rounded-lg overflow-x-auto overflow-y-hidden"
        ...
        {!isManualColoringMode && originalImageSrc && colorCounts && Object.keys(colorCounts).length > 0 && (
          // 白色卡片样式
          <div className="w-full md:max-w-2xl mt-6 bg-white p-4 rounded-xl shadow-lg color-stats-panel">
            {/* 标题和物料清单按钮 */}
            <div className="flex items-center justify-between mb-1">
              ...
              <button
                onClick={() => {
                  trackExportMaterialList(selectedColorSystem); // 追踪物料清单导出
                  exportMaterialList({
                    colorCounts,
                    totalBeadCount,
                    selectedColorSystem,
                ...
                className="px-3 py-1.5 bg-gradient-to-r from-red-500 to-pink-500 text-white text-sm rounded-lg hover:from-red-600 hover:to-pink-600 transition-all duration-200 shadow-sm hover:shadow-md flex items-center gap-1.5"
              >
            ...
            <p className="text-xs text-center text-gray-500 mb-3">
              ...
              {(() => {
                // 查找T01（白色）的数量
                const whiteColorEntry = Object.entries(colorCounts).find(([hexKey]) => {
                  const displayKey = getColorKeyByHex(hexKey, selectedColorSystem);
                  return displayKey === 'T01' || displayKey === 'T1';
                });
                const whiteCount = whiteColorEntry ? whiteColorEntry[1].count : 0;
                const actualNeeded = totalBeadCount - whiteCount;
                  ...
                  return (
                    <li
                      ...
                      className={`flex items-center justify-between p-1.5 rounded cursor-pointer transition-colors ${ 
                        isExcluded
                          ? 'bg-red-50 hover:bg-red-100 opacity-60' // 排除的颜色
                          : 'hover:bg-gray-50'
                      }`}
                    ...
                    >
                      <div className={`flex items-center space-x-2 ${isExcluded ? 'line-through' : ''}`}>
                        ...
                        ></span>
                        {/* 色号 */}
                        <span className={`font-mono font-medium ${isExcluded ? 'text-red-600' : 'text-gray-800'}`}>{displayColorKey}</span>
                      </div>
                      {/* 数量 */}
                      <span className={`text-xs ${isExcluded ? 'text-red-500 line-through' : 'text-gray-600'}`}>{count} 颗</span>
                    </li>
                  );
                })}
            </ul>
            {excludedColorKeys.size > 0 && (
                <div className="mt-3">
                  <button
                    ...
                    className="w-full text-xs py-1.5 px-2 bg-gray-100 text-gray-700 rounded hover:bg-gray-200 transition-colors flex items-center justify-between"
                  >
                    <span>已排除的颜色 ({excludedColorKeys.size})</span>
                    <svg 
                      xmlns="http://www.w3.org/2000/svg" 
                      className={`h-4 w-4 text-gray-500 transform transition-transform ${showExcludedColors ? 'rotate-180' : ''}`}
                      fill="none" 
                      viewBox="0 0 24 24" 
                      stroke="currentColor"
                    >
                        ...
                        {Array.from(excludedColorKeys).length > 0 ? (
                          <ul className="space-y-1">
                            {Array.from(excludedColorKeys).sort(sortColorKeys).map(hexKey => {
                              ...
                              return (
                                <li key={hexKey} className="flex justify-between items-center p-1 hover:bg-gray-100 rounded">
        ...
        {!isManualColoringMode && originalImageSrc && mappedPixelData && (
            <div className="w-full md:max-w-2xl mt-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                {/* 简洁版下载按钮 */}
                <button
                  onClick={handleSimpleDownloadRequest}
                  disabled={!mappedPixelData || !gridDimensions || gridDimensions.N === 0 || gridDimensions.M === 0 || activeBeadPalette.length === 0}
                  className="py-2.5 px-4 bg-gradient-to-r from-pink-500 to-pink-600 text-white text-sm sm:text-base rounded-lg hover:from-pink-600 hover:to-pink-700 focus:outline-none focus:ring-2 focus:ring-pink-500 focus:ring-offset-2 transition-all duration-300 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed shadow-md hover:shadow-lg hover:translate-y-[-1px] disabled:hover:translate-y-0 disabled:hover:shadow-md"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>
                  简洁版图纸
                </button>
        ...
        <div className="w-full flex justify-center mt-12 mb-8">
          <button
            ...
            className="group relative px-6 py-3 bg-gradient-to-r from-amber-400 via-orange-400 to-pink-400 text-white font-medium rounded-full shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 flex items-center gap-2"
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
